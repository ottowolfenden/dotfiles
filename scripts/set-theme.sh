...
if [ $mode = "dark" ]; then
    ...
    awww img $("$HOME"/dotfiles/scripts/daily-wallpaper.sh dark) --transition-type none
    ...
    awww img $("$HOME"/dotfiles/scripts/daily-wallpaper.sh light) --transition-type none
else
    exit 1
fi
